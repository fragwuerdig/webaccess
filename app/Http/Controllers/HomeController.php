...
class HomeController extends Controller
{
    ...
    {
		$users = \App\VirtualUser::all();
		$domains = \App\VirtualDomain::all();
		//return $users;
	...
	public function users_create(Request $request)
	{
		...
		$domain = \App\VirtualDomain::where('name', $post['domain'])->first();
		
		$user =  new \App\VirtualUser;
	...
	public function aliases_add(Request $request)
	{
		...
		$existing_alias = \App\VirtualAlias::where('source', $source . "@" . $domain)->get();
		$domain = \App\VirtualDomain::where('name', $domain)->first();

		if ($existing_alias->isEmpty()) {
			$alias = new \App\VirtualAlias;
